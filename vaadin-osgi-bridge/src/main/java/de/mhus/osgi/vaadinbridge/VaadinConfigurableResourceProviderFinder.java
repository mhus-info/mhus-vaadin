...
public class VaadinConfigurableResourceProviderFinder {
    ...
    public static VaadinConfigurableResourceProviderAdmin doFind(BundleContext context) {
        ServiceReference<VaadinConfigurableResourceProviderAdmin> ref =
                context.getServiceReference(VaadinConfigurableResourceProviderAdmin.class);
    ...
    public static void add(final BundleContext context, final String... pathes) {
                    ...
                    new ServiceListener() {
                        ...
                        public void serviceChanged(ServiceEvent event) {
                            ...
                            if (event.getType() == ServiceEvent.REGISTERED) {
                                ...
                                VaadinConfigurableResourceProviderAdmin service =
                                        (VaadinConfigurableResourceProviderAdmin)
                                                context.getService(event.getServiceReference());
                                if (service != null)
                                    service.addResource(
                                            context.getBundle().getSymbolicName(), pathes);
                            }
                        }
                    },
                    "(objectclass="
                            + VaadinConfigurableResourceProviderAdmin.class.getCanonicalName()
                            + ")");
        ...
        if (service != null) service.addResource(context.getBundle().getSymbolicName(), pathes);
    }
}
