...
public class LoginScreen extends CssLayout implements MNlsProvider {
    ...
    private Component buildLoginForm() {
        ...
        loginForm.addComponent(
                username = new TextField(MNls.find(this, "login.username=Username"), ""));
        username.setWidth(15, Unit.EM);
        loginForm.addComponent(
                password = new PasswordField(MNls.find(this, "login.password=Password")));
    ...
    private CssLayout buildLoginInformation() {
        ...
        Label loginInfoText =
                new Label(MNls.find(this, "login.info.text=Content Editor"), ContentMode.HTML);
    ...
    private void login() {
        if (accessControl.signIn(username.getValue(), password.getValue())) {
            ...
            Subject subject =
                    (Subject)
                            UI.getCurrent()
                                    .getSession()
                                    .getAttribute(VaadinAccessControl.SUBJECT_ATTR);
            if (subject == null) MLogUtil.log().d("no subject for login");
            else MLogUtil.log().d("Login", subject);
        ...
        showNotification(
                new Notification(
                        MNls.find(this, "login.error.title=Login failed"),
                        MNls.find(
                                this,
                                "login.error.text=Please check your username and password and try again."),
