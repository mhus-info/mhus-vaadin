...
public class LayUtil {
    ...
    public static void configure(AbstractComponent layout, INode config) throws MException {
        ...
        if (layout instanceof Layout.SpacingHandler && config.isProperty(LayoutBuilder.SPACING))
            ((Layout.SpacingHandler) layout)
                    .setSpacing(config.getBoolean(LayoutBuilder.SPACING, false));
        // styles
        if (config.isProperty(LayoutBuilder.STYLE)) {
            layout.setStyleName(config.getExtracted(LayoutBuilder.STYLE));
        }
        // hidden
        if (config.getBoolean(LayoutBuilder.HIDDEN, false)) layout.setVisible(false);
        ...
        if (layout instanceof AbstractSplitPanel) {
            ...
            if (a != null) {
                ...
                if (s[0] >= 0)
                    ((AbstractSplitPanel) layout)
                            .setMinSplitPosition(s[0], Unit.values()[(int) s[1]]);
            }
            ...
            if (a != null) {
                ...
                if (s[0] >= 0)
                    ((AbstractSplitPanel) layout)
                            .setMaxSplitPosition(s[0], Unit.values()[(int) s[1]]);
            }
