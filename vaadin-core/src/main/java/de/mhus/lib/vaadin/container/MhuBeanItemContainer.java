 ...
 * JavaBean class. Only beans of the same type can be added to the container.
 *
 * <p>BeanItemContainer uses the beans themselves as identifiers. The {@link Object#hashCode()} of a
 * bean is used when storing and looking up beans so it must not change during the lifetime of the
 * bean (it should not depend on any part of the bean that can be modified). Typically this
 * restricts the implementation of {@link Object#equals(Object)} as well in order for it to fulfill
 * the contract between {@code equals()} and {@code hashCode()}.
 *
 * <p>To add items to the container, use the methods {@link #addBean(Object)}, {@link
 * #addBeanAfter(Object, Object)} and {@link #addBeanAt(int, Object)}. Also {@link
 * #addItem(Object)}, {@link #addItemAfter(Object, Object)} and {@link #addItemAt(int, Object)} can
 * be used as synonyms for them.
 *
 * <p>It is not possible to add additional properties to the container.
 *
...
public class MhuBeanItemContainer<BEANTYPE> extends MhuAbstractBeanContainer<BEANTYPE, BEANTYPE> {
    ...
    public void configureTableByAnnotations(Table table, String schema, MNls nls) {
        ...
        for (String colId : columns.values()) {
            ...
            if (title == null && !MString.isEmpty(columnDef.title())) title = columnDef.title();
            if (title != null) table.setColumnHeader(colId, title);
     ...
     *
     * <p>This corresponds to the old behavior of {@link MhuBeanItemContainer}, and requires
     * suitable (identity-based) equals() and hashCode() methods on the beans.
     *
     * @param <BT>
     * @since 6.5
     */
    ...
    /**
     * Constructs a {@code BeanItemContainer} and adds the given beans to it. The collection must
     * not be empty. MhuBeanItemContainer BeanItemContainer(Class) can be used for creating an
     * initially empty {@code BeanItemContainer}.
     *
     * <p>Note that when using this constructor, the actual class of the first item in the
     * collection is used to determine the bean properties supported by the container instance, and
     * only beans of that class or its subclasses can be added to the collection. If this is
     * problematic or empty collections need to be supported, use BeanItemContainer(Class) and
     * {@link #addAll(Collection)} instead.
     *
     * @param collection a non empty {@link Collection} of beans.
    ...
    /**
     * Internal helper method to support the deprecated {@link Collection} container.
     *
     * @param <BT>
     * @param collection
     * @return
    ...
    private static <BT> Class<? extends BT> getBeanClassForCollection(
            Collection<? extends BT> collection) throws IllegalArgumentException {
     ...
     *
     * @param type the type of the beans that will be added to the container.
     * @param collection a {@link Collection} of beans (can be empty or null).
     * @throws IllegalArgumentException If {@code type} is null
     */
    public MhuBeanItemContainer(
            Class<? super BEANTYPE> type, Collection<? extends BEANTYPE> collection)
            throws IllegalArgumentException {
        super(type);
        super.setBeanIdResolver(new IdentityBeanIdResolver<BEANTYPE>());
    ...
    /**
     * Adds all the beans from a {@link Collection} in one go. More efficient than adding them one
     * by one.
     *
     ...
     *     <p>com.vaadin.data.Container.Ordered#addItemAfter(Object, Object)
     */
    @Override
    @SuppressWarnings("unchecked")
    public MhuBeanItem<BEANTYPE> addItemAfter(Object previousItemId, Object newItemId)
            throws IllegalArgumentException {
     ...
     * <p>The bean is used both as the item contents and as the item identifier.
     *
     * <p>com.vaadin.data.Container#addItem(Object)
     */
    @Override
    @SuppressWarnings("unchecked")
