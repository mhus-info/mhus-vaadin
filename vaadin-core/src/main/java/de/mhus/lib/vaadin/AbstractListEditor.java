...
public abstract class AbstractListEditor<E> extends VerticalLayout implements MNlsProvider, ILog {
    ...
    @Hidden private Log log = Log.getLog(this);
        ...
        table.addHeaderClickListener(
                new Table.HeaderClickListener() {
                    ...
                    public void headerClick(HeaderClickEvent event) {
                        ...
                        if (name.equals(sortedColumn)) sortedAscending = !sortedAscending;
                        else sortedAscending = true;
                        sortedColumn = name;
                        if (needSortUpdate) updateDataSource();
                    }
                });
    ...
    protected void doItemClick(ItemClickEvent event) {
        if (editMode == null && event.isDoubleClick()) doUpdate();
    }
    ...
    protected void composeElements(
            AbstractComponent filter,
            AbstractComponent table,
            AbstractComponent formPanel,
            AbstractComponent buttonBar) {
    ...
    protected VaadinPojoForm<E> createForm() {
        try {
            VaadinPojoForm<E> form =
                    new VaadinPojoForm<>(createTarget()) {
                        private static final long serialVersionUID = 1L;
    ...
    protected void doSelectionChanged() {
        ...
        E target = null;
        if (selectedId == null) target = createTarget();
        else target = getTarget(selectedId);
    ...
    protected void doDelete() {
        ...
        ConfirmDialog.show(
                getUI(),
                MNls.find(this, "confirm.delete=Delete"),
                MNls.find(this, "confirm.question=Are you sure?"),
                MNls.find(this, "confirm.yes=Yes"),
                MNls.find(this, "confirm.no=Cancel"),
    ...
    /**
     * Overwrite this function if you want to validate a new entity before saving. Return null if
     * the values are correct or return the internationalized error message.
     *
     * @param entity
     * @return
     */
    ...
    /**
     * Overwrite this function if you want to validate an updated entity before saving. Return null
     * if the values are correct or return the internationalized error message.
     *
     * @param entity
     * @return
     */
    ...
    protected void updateEnabled() {
        ...
        if (!isEditMode()) {
            ...
            if (info != null) info.setVisible(true);
    ...
    public void updateDataSource() {
        try {
            ...
            for (E entity : createDataList(filter.createFilterRequest())) {
                ...
                if (!table.updateRow(id, values)) table.addRow(id, values);
                newIds.add(id);
            }

            for (Object id : new LinkedList<Object>(table.getItemIds())) {
                if (!newIds.contains(id)) table.removeItem(id);
            }
    ...
    public E getSingleSelected() {
        if (isEditMode()) return getTarget(editMode);
        Object selectedId = table.getValue();
    ...
    @Override
    public MNls getNls() {
        if (nlsBundle == null) nlsBundle = MNlsBundle.lookup(this);
