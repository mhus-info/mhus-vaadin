...
public class UiLink extends UiVaadin {
    ...
    protected void setValue(Object value) throws MException {
        ...
        String label =
                MXml.encode(
                        ds.getString(this, "label", getConfig().getString("label", "label=Link")));
        ((Label) getComponentEditor()).setCaptionAsHtml(true);
        if (value == null) ((Label) getComponentEditor()).setCaption(label);
        else {
            String link = MXml.encode(String.valueOf(value));
            ((Label) getComponentEditor())
                    .setCaption("<a href=\"" + link + "\" target=_blank>" + label + "</a>");
        }
        this.value = value;
    }
