...
public class MhuTable extends ExpandingTable {
    ...
    //	protected static final Action ACTION_ENTER = new ShortcutAction("Enter",
    // ShortcutAction.KeyCode.ENTER,null);
    //	protected static final Action ACTION_ESCAPE = new
    // ShortcutAction("Escape",ShortcutAction.KeyCode.ESCAPE,null);
        ...
        setTableFieldFactory(
                new TableFieldFactory() {
                    ...
                    public Field<?> createField(
                            ...
                            Component uiContext) {
                        Field<?> f =
                                MhuTable.this.createField(container, itemId, propertyId, uiContext);
                        return f;
                    }
                });
    ...
    protected Field<?> createField(
        ...
        if (f instanceof AbstractField) {
            ...
            if (model.getConverter() != null)
                ((AbstractField<String>) f)
                        .setConverter((Converter<String, ?>) model.generateConverter(type));
        }
        return f;
    }
    ...
    public static Class<?> findDefaultConverter(ColumnModel model, Class<?> type) {
        ...
        if (type == int.class) return IntPrimitiveConverter.class;
        if (type == boolean.class) return BooleanPrimitiveConverter.class;
        if (type == long.class) return LongPrimitiveConverter.class;
        if (type == double.class) return DoublePrimitiveConverter.class;
        if (type == float.class) return FloatPrimitiveConverter.class;

        if (type == Integer.class) return StringToIntegerConverter.class;
        if (type == Time.class) return SqlTimeConverter.class;
        if (type == DateTime.class) return DateTimeConverter.class;
        if (type == java.sql.Date.class) return SqlDateConverter.class;
        if (type == java.util.Date.class) return DateConverter.class;
        if (type == Boolean.class) return StringToBooleanConverter.class;
        if (type == Long.class) return StringToLongConverter.class;
        if (type == Double.class) return StringToDoubleConverter.class;
        if (type == Float.class) return StringToFloatConverter.class;
