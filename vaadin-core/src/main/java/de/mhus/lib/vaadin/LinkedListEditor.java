...
public class LinkedListEditor<E> extends AbstractBeanListEditor<E> {
    ...
    protected E createTarget() {
        try {
            ...
            if (out instanceof ManagedListEntity) ((ManagedListEntity) out).doPostCreate(this);
            return out;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void doCancel(E entry) {
        // nothing to do
        if (entry instanceof ManagedListEntity) ((ManagedListEntity) entry).doCancel(this);
    }

    @Override
    public void doDelete(E entry) {
        if (entry instanceof ManagedListEntity) ((ManagedListEntity) entry).doPreDelete(this);
        if (created.contains(entry)) created.remove(entry);
        else deleted.add(entry);
        list.remove(entry);
        if (entry instanceof ManagedListEntity) ((ManagedListEntity) entry).doPostDelete(this);
        setModified(true);
    }
    ...
    public void doSave(E entry) {
        ...
        if (entry instanceof ManagedListEntity) ((ManagedListEntity) entry).doPreSave(this);
        ...
        if (!created.contains(original)) changed.add(original);

        if (original instanceof ManagedListEntity) ((ManagedListEntity) original).doPostSave(this);
    ...
    protected E getTarget(Object id) {
        for (E item : list) if (getId(item).equals(id)) return item;
        return null;
    }
