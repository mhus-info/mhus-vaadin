...
public class Desktop extends CssLayout implements MNlsProvider {
    ...
    private HashMap<String, AbstractComponent> spaceInstanceList =
    ...
    protected void initGui() {
        ...
        menuLogout =
                menuUser.addItem(
                        MNls.find(this, "menu.logout=Logout"),
                        new MenuBar.Command() {
    ...
    public void refreshSpaceList() {
        ...
        menuOverview =
                menuSpaces.addItem(
                        MNls.find(this, "menu.overview=Overview"),
                        new MenuBar.Command() {
                        ...
                        new NativeButton.ClickListener() {
                            ...
                            public void buttonClick(ClickEvent event) {
                                openSpace(
                                        space.getName(),
                                        null,
                                        null); // not directly to support history
                            }
                        });
                tile = button;
        ...
        if (componentList.size() > 0) menuSpaces.addSeparator();

        menuLeave =
                menuSpaces.addItem(
                        MNls.find(this, "menu.leave=Leave Space"),
                        new MenuBar.Command() {
    ...
    protected String showSpace(GuiSpaceService space, String subSpace, String search) {
        ...
        cleanupMenu();
        if (component == null) {
            contentScreen.addComponent(
                    new Label(
                            MNls.find(this, "spaces.unavailable=Space currently not available")
                                    + space.getName()));
            addComponent(contentScreen);
            return null;
        }
        ...
        if (component instanceof Navigable) {
            ...
            else ((Navigable) component).onShowSpace(!exists);
        }
    ...
    protected void showOverview(boolean setLinking) {
        ...
        if (setLinking) UI.getCurrent().getPage().setUriFragment("");
    }
    ...
    public boolean openSpace(
        ...
        if (!space.hasAccess(getApi().getAccessControl()))
            //        if (!getApi().hasAccess(space.getName()) ||
            // !space.hasAccess(getApi().getAccessControl()))
            return false;
