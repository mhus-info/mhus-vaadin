...
public class MVaadin {
    ...
    /**
     * Handles the ENTER key while the TextField has focus. See
     * https://vaadin.com/forum/#!/thread/77601/7736664 See
     * http://ramontalaverasuarez.blogspot.co.at/2014/06/vaadin-7-detect-enter-key-in-textfield.html
     *
     * @param searchField
     * @param listener e.g. TextField searchField = new TextField();
    ...
    public static void handleEnter(TextField searchField, Listener listener) {
        ShortcutListener enterShortCut =
                new ShortcutListener("EnterShortcut", ShortcutAction.KeyCode.ENTER, null) {
    ...
    public static void handleShortcut(
            AbstractTextField textField, ShortcutListener shortcutListener) {
        textField.addFocusListener(
                (FieldEvents.FocusEvent event) -> textField.addShortcutListener(shortcutListener));
        textField.addBlurListener(
                (FieldEvents.BlurEvent event) ->
