...
public class UiDate extends UiVaadin {
    ...
    protected void setValue(Object value) throws MException {
        ...
        if (date == null) {
            ...
            LocalDateTime localDate =
                    Instant.ofEpochMilli(date.getTime()).atZone(zoneId).toLocalDateTime();
    ...
    public Component createEditor() {
        ...
        FORMATS format =
                FmDate.FORMATS.valueOf(
                        getConfig().getString("format", FmDate.FORMATS.DATE.name()).toUpperCase());
        switch (format) {
            case DATE:
                ret = new DateField();
            ...
            case CUSTOM:
                ...
                if (custom != null) ret.setDateFormat(custom);
                ((DateTimeField) ret).setResolution(DateTimeResolution.SECOND);
                break;
            default:
                ret = new DateField();
