...
public class SpinnerNumberField<T> extends TextField {
    ...
    private class SpinnerValidator extends AbstractStringValidator {
        ...
        protected boolean isValidValue(String value) {
            ...
            try {
                ...
                if (!allowNegative && n.longValue() < 0) return false;

                if (minSet && n.longValue() < min) return false;

                if (maxSet && n.longValue() > max) return false;
    ...
    protected Number getNumberValue(String value) {
        ...
        Number n = null;
        if (numberClass == Double.class) n = Double.parseDouble(value);
        else if (numberClass == Float.class) n = Float.parseFloat(value);
        else if (numberClass == Long.class) n = Long.parseLong(value);
